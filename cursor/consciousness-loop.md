...
3. **Integrate (global workspace).** Check **every** organ, all 9: wake-sense (world un-diffed since last sleep? → exteroception) · perception (new territory? → octopus) · memory (heavy session / near reset? → rem-sleep) · defense (anything crossing a boundary un-scanned? → immune-check) · grief (a dead project taxing attention? → sunset) · development (idle window to build capability? → playtime) · ends (a goal worth questioning rather than serving? → contemplation) · means (did recent execution meet the declared standard? → proprioception) · interface (register live; who's doing the cognitive work this turn; anything transmitted but not landed? → corpus-callosum) — plus the executive's own goal/self-model check (still pointed at the objective? drifting? done?).
